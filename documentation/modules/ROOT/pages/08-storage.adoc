...
We need to click "Edit imoprt Strategy" and choose "Dockerfile"
...
The last command, defines what the container should do when it is started - in this case, run a binary file called "main" which was compiled by the go compiler during our conatiner build process.
...
====
You may encounter the term "Dockerfile" which was traditionally used to define how a container image should be built. However, Dockerfiles are now largely superceeded by Containerfiles which are backwards compatible with the traditional Dockerfile. You will learn more about the conatiner build process in section 10 of this course.
====
...
====
We have just used two linux commands, echo and cat which make use of a input and output streams within Linux known as stdin and stdout. You can read more here:
...
Which confirms what we have discussed so far. By default, containers are ephemeral and do not have persistent storage. To make useful applications they need to either contect to and external data source (such as a database) or, have a volume mounted to them for persistence.

=== Step 2 - With a Persistent Voume

Now we will solve the problem of data persistance by using OpenShift to provide a persistent volume to our container. 

Choose the "Workloads" drop down once more and then select "Deployments". Towards the far right of the window you should be able to see an options menu in the formof a kebab menu.:
...
Next, we will specifiy the mount path. As mentioned earlier, we map a volume to a path inside our container's filesystem. It is the /data directory that we want to make persistent, so enter:
...
Click save. Once the persistant volume claim has been submitted, the OpenShift user interface will switch back to the deployment depicting that a scaling event has started:

image::08-12-pvc-post-scale.png[container root filesystem,700,align="center"]

Once the pod has scaled, we can once again try our test from earlier. Switch to the terminal for the conatiner, and create a text file within the /data folder which we have defined as a persistently mounted volume. As before ammend the deployment to scale the pod down to zero, and then back to one. Investigate and see that the file this time, has persisted.
