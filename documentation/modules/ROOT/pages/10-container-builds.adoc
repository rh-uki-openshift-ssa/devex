...
Select "Copy login command" and provide the username and password that you were given at the start of the workshop. From the new browser tab window select "Display Token" which will show you the following information (with your specific token) :
   ...
   - name: priv
     ...
     securityContext:
       ...
       spec:
        containers:
          - name: priv
            image: quay.io/marrober/devex-podman:1
            args:
              - sleep
              - "1000000"
            securityContext:
